...
from terrain_smoothing import smooth_terrain_boundaries
from meta_data_process import capture_metadata, add_metadata
...
def create_rotated_stl_from_dem(dem_path, output_stl, crop_km, rotation_deg, center_lat, center_lon,smooth_terrain=True, debug = False, intermediate_save=True,):
    ...
    try:
        ...
        with rasterio.open(dem_path) as src:
            ...
            add_metadata("original_crs", str(src.crs))
            add_metadata("original_bounds", list(src.bounds))
            add_metadata("original_shape", src.shape)
            ...
            if src.crs == CRS.from_epsg(4326) or 'geographic' in str(src.crs).lower():
                ...
                if intermediate_save:
                    ...
                    utm_dem_path, utm_crs = reproject_to_utm(debug, dem_path)
                # Add reprojection metadata
                add_metadata("reprojected_to_utm", True)
                add_metadata("utm_crs", str(utm_crs))
            else:
                print_debug("DEM appears to already be in projected coordinates", debug)
                add_metadata("reprojected_to_utm", False)
...
def process_dem(config_override=None):
    ...
    if config_override:
        ...
        from dem_processing.process_config import ProcessConfig
    ...
    smooth_terrain_boundaries(
                                stl_file, 
                                domain_size=30000, 
                                aoi_size=8000,
                                plot=config.debug_mode
                            )
